...
export function MonthlyMileageDialog({
  open,
  onOpenChange,
  vehicles,
}: {
  ...
  useEffect(() => {
    if (!open) return;
    setVehicleId(
      (current) => current || (eligible[0] ? String(eligible[0].id) : ""),
    );
      ...
      await save.mutateAsync({
        ...
        passengers:
          selected?.vehicle_type === "car" || selected?.vehicle_type === "van"
            ? passengerCount
            : 0,
        notes: `Monthly total · ${notes.trim()}`.replace(/ · $/, ""),
      });
      onOpenChange(false);
    } catch (caught) {
      setError(
        caught instanceof Error
          ? caught.message
          : "Monthly mileage could not be saved.",
      );
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Add monthly mileage</DialogTitle>
          <DialogDescription>
            Record one total for a month when you do not want to log every
            journey. Keep supporting notes or a calendar record if needed.
          </DialogDescription>
        </DialogHeader>
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div>
            <Label>Total business miles</Label>
            <Input
              type="number"
              min="0.1"
              step="0.1"
          ...
          <div>
            ...
            <Input
              ...
              value={passengers}
              disabled={
                !selected || !["car", "van"].includes(selected.vehicle_type)
              }
          ...
          <div className="sm:col-span-2">
            ...
            <Textarea
              ...
              placeholder="Regular visits to customer sites"
            />
          </div>
        </div>
        {distance > 0 && (
          <p className="rounded-md bg-muted p-3 text-sm text-muted-foreground">
            Indicative allowance before the shared car/van tier:{" "}
            <strong className="text-foreground">
              £{(distance * approximateRate).toFixed(2)}
            </strong>
            . The final amount is recalculated against your tax-year mileage
            total.
          </p>
        )}
        ...
        <DialogFooter>
          ...
          >
            {save.isPending ? "Saving..." : "Save monthly total"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
